...
fn test_trexecutor_with_bad_code() {
    ...
    good_trans.set_total_fees(302659103u32.into());
...
fn test_trexecutor_active_acc_with_code1() {
    ...
    let executor = OrdinaryTransactionExecutor::new(config);
    let trans = executor.execute(Some(&msg), &mut acc_root, BLOCK_UT, BLOCK_LT, lt.clone(), true).unwrap();
